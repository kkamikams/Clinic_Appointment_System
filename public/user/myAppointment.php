...
$today        = date('Y-m-d');
$filterStatus = in_array($_GET['status'] ?? '', ['', 'upcoming', 'Pending', 'In Progress', 'Completed', 'Cancelled'])
    ? ($_GET['status'] ?? '') : '';

$data = getMyAppointmentsData($conn, $userId, $today);
extract($data);
...
<section class="section page-myappt">
    ...
    <div class="stat-strip">
        ...
        <a href="my_appointment?status=Completed" class="stat-card">
            <div class="sc-label">Completed</div>
            <div class="sc-num"><?= $statCompleted ?></div>
            <div class="sc-sub">Finished visits</div>
        </a>
        <a href="my_appointment?status=Cancelled" class="stat-card">
    ...
    <div class="main-card">
        <div class="table-toolbar">
            ...
            <select class="filter-select" id="statusFilter" onchange="filterTable()">
                ...
                <option value="Pending" <?= $filterStatus === 'Pending'      ? 'selected' : '' ?>>Upcoming</option>
                <option value="In Progress" <?= $filterStatus === 'In Progress'  ? 'selected' : '' ?>>In Progress</option>
                <option value="Completed" <?= $filterStatus === 'Completed'    ? 'selected' : '' ?>>Completed</option>
                <option value="Cancelled" <?= $filterStatus === 'Cancelled'    ? 'selected' : '' ?>>Cancelled</option>
            </select>
            <a href="bookAppointment" class="btn-book"><i class="bi bi-plus-lg"></i> Book New</a>
        </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        document.getElementById('cancelConfirmBtn').addEventListener('click', () => {
            ...
            fetch('../../app/controllers/bookappHandler.php?action=cancel_appointment', {
                    method: 'POST',
                    body: fd
                })
                .then(r => r.json())
